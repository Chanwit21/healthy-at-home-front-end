...
function ManageProductAddProduct({ setAlertMessage, addProductForm }) {
  const [addProduct, setAddProduct] = useState({ name: '', title: '', price: '', day: '' });
  const [error, setError] = useState({ name: '', title: '', price: '', day: '', image1: '', image2: '' });
  ...
  const handleSubmitForm = (e) => {
    ...
    for (let key of Object.keys(addProduct)) {
      if (!addProduct[key]) {
        allPass = false;
        setError((cur) => {
          const clone = { ...cur };
    ...
    if (allPass) {
      ...
      for (let key of Object.keys(addProduct)) {
        form.append(key, addProduct[key]);
      }
    ...
    <div className='manage_product_card'>
      <form onSubmit={handleSubmitForm}>
        ...
        <div className='card_row_title'>
          <p>Name :</p>
          <p>
            <input type='text' value={addProduct.name} onChange={(e) => handleChangeInput('name', e)} />
          </p>
        </div>
        {error.name ? (
          <div className='invalid-text' style={{ textAlign: 'center' }}>
        ...
        <div className='card_row_title'>
          <p>Title :</p>
          <p>
            <input type='text' value={addProduct.title} onChange={(e) => handleChangeInput('title', e)} />
          </p>
        </div>{' '}
        {error.title ? (
          <div className='invalid-text' style={{ textAlign: 'center' }}>
        ...
        <div className='card_row_title'>
          <p>Price :</p>
          <p>
            <input type='text' value={addProduct.price} onChange={(e) => handleChangeInput('price', e)} />
          </p>
        </div>
        {error.price ? (
          <div className='invalid-text' style={{ textAlign: 'center' }}>
        ...
        <div className='card_row_title'>
          <p>Day :</p>
          <p>
            <input type='text' value={addProduct.day} onChange={(e) => handleChangeInput('day', e)} />
          </p>
        </div>
        {error.day ? (
          <div className='invalid-text' style={{ textAlign: 'center' }}>
...
export default ManageProductAddProduct;
